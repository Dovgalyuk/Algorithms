...
void searchAlgorithm(int rowSize, int colSize, int row, int col, char** maze, int** visitedCell, int** path, Queue*& map)
{
	...
	{
		if ((row + 1) < rowSize && !visitedCell[row + 1][col] && (maze[row + 1][col] == '.' || maze[row + 1][col] == 'X'))
		{
			path[row + 1][col] = path[row][col] + 1;
			queue_insert(map, row + 1);
			queue_insert(map, col);
		}
		if ((row - 1) >= 0 && !visitedCell[row - 1][col] && (maze[row - 1][col] == '.' || maze[row - 1][col] == 'X'))
		{
			path[row - 1][col] = path[row][col] + 1;
			queue_insert(map, row - 1);
			queue_insert(map, col);
		}
		if ((col + 1) < colSize && !visitedCell[row][col + 1] && (maze[row][col + 1] == '.' || maze[row][col + 1] == 'X'))
		{
			path[row][col + 1] = path[row][col] + 1;
			queue_insert(map, row);
			queue_insert(map, col + 1);
		}
		if ((col - 1) >= 0 && !visitedCell[row][col - 1] && (maze[row][col - 1] == '.' || maze[row][col - 1] == 'X'))
		{
			path[row][col - 1] = path[row][col] + 1;
			queue_insert(map, row);
			queue_insert(map, col - 1);
