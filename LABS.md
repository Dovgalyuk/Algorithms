...
Второй вариант тестирования - это сравнение файла, сгенерированного программой,
с образом. Если программа записывает результат в файл с именем, переданным в командной строке, а правильный вывод
находится в файле valid.txt, то тест будет выглядеть так:

    add_test(NAME TestLab1C COMMAND Lab1C output.txt)
    add_test(NAME TestLab1C_ok COMMAND ${CMAKE_COMMAND} -E compare_files output.txt ${CMAKE_CURRENT_SOURCE_DIR}/valid.txt)

Первый тест соответствует запуску программы, а второй сравнению ее вывода с эталоном.
