...
int main() {
    ...
    printf("Количество элементов, превышающих максимальное значение по абсолютной величине: %d\n", count);
    array_destroy(arr1);

    printf("Задача 5\n");
    file = fopen("input_test2.txt", "r");
    if (!file) {
        printf ("Ошибка открытия файла задачи 5\n");
        return 1;
    }
    ...
    if (start_index != -1) {
        printf("Максимальная сумма 5 последовательных элементов: %d\n", max_sum);
        ...
        printf("Массив слишком мал для 5 последовательных элементов\n");
    }
    array_destroy(arr2);
