...
void process_task1(const char* filename) {
    ...
    for (size_t i = 0, j = 0; i < n; ++i) if (arr[i] % 2 == 0) even_indices[j++] = static_cast<int>(i);

    for (size_t i = 0; i < even_indices.size(); ++i) std::cout << even_indices[i] << (i + 1 == even_indices.size() ? "" : " ");
    std::cout << "\n";
}
