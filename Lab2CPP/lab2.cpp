...
void read_numbers_and_commands_from_file(const char* filename, std::vector<CommandItem>& cmds) {
    ...
    while (file >> token) {
        ...
        if (cmd_code != cmd_none) {
            cmds.push_back({ cmd_code, 0 });
        }
        else {
            int num = std::stoi(token);
            cmds.push_back({ cmd_push, num });
        }
    }
}

size_t skip_cond_block(const std::vector<CommandItem>& cmds, size_t current_index, size_t count) {
    size_t nested = 0;
    size_t i = current_index + 1;
    while (i < count) {
        if (cmds[i].cmd == cmd_cond) nested++;
        else if (cmds[i].cmd == cmd_end) {
            if (nested == 0) break;
            nested--;
        }
        i++;
...
void execute_commands(const std::vector<CommandItem>& cmds, Stack* nums, std::ifstream& inputnum) {
    int loop_var = 0;
    size_t i = 0;
    size_t count = cmds.size();

    while (i < count) {
        CommandItem cmdItem = cmds[i];
        switch (cmdItem.cmd) {

        case cmd_push:
            stack_push(nums, cmdItem.value);
            ++i;
            break;
        ...
        case cmd_repeat:
            if (loop_var > 0) {
                loop_var--;
                ssize_t j = i - 1;
                while (j >= 0 && cmds[j].cmd != cmd_setr) j--;
                i = (j >= 0) ? j + 1 : i + 1;
            }
            else {
                ++i;
...
int main(int argc, char* argv[]) {
    ...
    read_numbers_and_commands_from_file(argv[1], cmds);
