...
//Первый код использует дополнительные массивы для отслеживания посещенных ячеек(tag_map) и расстояний(routes).
//Это может замедлить код, так как каждый раз при доступе к ячейке лабиринта также осуществляется доступ к соответствующим ячейкам в этих массивах.
//Второй код использует только один дополнительный массив(rast) для хранения расстояний.