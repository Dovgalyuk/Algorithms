...
int main() {
    ...
    Graph<int> *graph = new Graph<int>(numberVertexes, 1);
    ...
    if (!graph->whetherEdge(graph->getVertex(0), graph->getVertex(5))) {
    ...
    if (!graph->whetherEdge(graph->getVertex(0), graph->getVertex(3)) || graph->getWeightEdge(graph->getVertex(0), graph->getVertex(3)) != 5) {
        std::cout << "Adding a edge does not work correctly.";
        return 1;
    }
    ...
    if (graph->whetherEdge(graph->getVertex(0), graph->getVertex(9))) {
