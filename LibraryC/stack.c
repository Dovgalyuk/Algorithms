...
Stack *stack_create(FFree f)  // Функция для создания нового стека
{
    ...
    stack->vector = vector_create(NULL);  // Создаем вектор с начальным размером 4 для стека
    ...
    stack->distruct = f; // Установка функции деструктора
...
void stack_delete(Stack *stack)  // Функция для удаления стека и освобождения памяти
{
    ...
    if (stack->distruct != NULL) { // Проверка наличия функции деструктора
