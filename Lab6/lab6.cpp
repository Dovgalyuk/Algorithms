...
int main(int argc, char** argv) {
    ...
    size_t a = 0;
    in >> a;
    if (a >= graph.get_vertices().size()) {
        std::cout << "Invalid index\n";
        return 1;
    }

    Vector<unsigned int> distances = find_min_path(graph, a);
    std::cout << "Distance from " << a << " to:\n";
...
bool less(const std::pair<unsigned int, size_t>& a, const std::pair<unsigned int, size_t>& b) {
    return a < b;
}
