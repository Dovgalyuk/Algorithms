...
// Функция проверки совпадения C-строк (аналог strcmp == 0)
bool strings_are_equal(const char* s1, const char* s2) {
    ...
    return *s1 == *s2; // Должны оба указывать на \0
}

// Функция проверки совпадения по длине (аналог strncmp == 0)
// Полезна для StringSlice, который может не иметь терминального нуля
bool buffer_is_equal(const char* buf, const char* expect, size_t len) {
...
void test_string_basics() {
    ...
    // 1. Создание
    String s1("Hello");
    ASSERT_TRUE(strings_are_equal(s1.data(), "Hello"), "Constructor with C-string");
    ASSERT_EQ_INT(s1.length(), 5, "Length check");

    // 2. Копирование
    String s2 = s1;
    ASSERT_TRUE(strings_are_equal(s2.data(), "Hello"), "Copy constructor data");
    ASSERT_TRUE(s1.data() != s2.data(), "Deep copy check (pointers must differ)");

    // 3. Присваивание
    String s3;
    s3 = s1;
    ASSERT_TRUE(strings_are_equal(s3.data(), "Hello"), "Assignment operator");
    
    // 4. Конкатенация
    String s4(" World");
    String s5 = s1 + s4;
    ASSERT_TRUE(strings_are_equal(s5.data(), "Hello World"), "Concatenation String + String");
}
...
void test_string_slice() {
    ...
    ASSERT_TRUE(buffer_is_equal(slice1.data(), "Heavy Metal", 11), "Slice from String");
    ASSERT_EQ_INT(slice1.length(), 11, "Slice length full");

    // 2. Частичный срез
    StringSlice slice2(str.data() + 6, 5);
    ASSERT_TRUE(buffer_is_equal(slice2.data(), "Metal", 5), "Partial Slice creation");
    ASSERT_EQ_INT(slice2.length(), 5, "Partial Slice length");

    // 3. Конкатенация String + Slice
    String prefix("I like ");
    String result = prefix + slice2; 
    ASSERT_TRUE(strings_are_equal(result.data(), "I like Metal"), "String + Slice concatenation");
    
    // 4. Конкатенация Slice + Slice
    StringSlice part1(str.data(), 5); // "Heavy"
    String full = part1 + slice2;     // "Heavy" + "Metal"
    ASSERT_TRUE(strings_are_equal(full.data(), "HeavyMetal"), "Slice + Slice concatenation");
}
...
void test_boyer_moore_search() {
    ...
    // Тест поиска в срезе
    StringSlice slice(text.data() + 1, 3); // "bra"
    idx = slice.find("ra"); 
