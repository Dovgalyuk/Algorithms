...
{
    size_t total_resistance = 0; //Переменная для хранения общего сопротивления
...
void task2(Array* arr, size_t shift_direction, size_t number_elements_shift)
{
    ...
    {
        for (size_t i = array_size(arr) - 1; i > 0; i--) //Перебор массива с конца
        {
    ...
    {
        for (size_t i = 0; i < array_size(arr) - 1; i++) //Перебор массива с начала
        {
            if (number_of_changes > 0) //Если количество изменений массива больше 0, то
            {
...
int main()
{
    ...
    int shift_direction; //Переменная для хранения направления сдвига
